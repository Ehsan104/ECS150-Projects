...
For this utility, first print the string File blocks with a newline at the end and then print each of the disk block numbers for the file to standard out, one disk block number per line. 
After printing the file blocks, print an empty line with only a newline.
Next, print the string File data with a newline at the end and then print the full contents of the file to standard out. 
You do not need to differentiate between files and directories, for this utility everything is considered to be data and should be printed to standard out.
*/
...
int main(int argc, char *argv[]) {
    ...
    int numBlocks = (inode.size / UFS_BLOCK_SIZE) + ((inode.size % UFS_BLOCK_SIZE) ? 1 : 0); //Calculate the number of blocks
    ...
    int ret = fs.read(inodeNumber, buffer, inode.size);

    if (ret < 0) {  //Check for errors
        return ret; 
    }

    buffer[inode.size] = '\0'; //Null terminate the buffer
    
    // Print File data
    cout << "File data" << endl; 
    
    if (inode.size > 0) {  //Print the buffer
        cout << buffer; 
    }
