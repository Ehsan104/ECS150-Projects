...
This program will start at the root of the file system, print the contents of that directory in full, and then traverse to each directory contained within. 
This process repeats in a depth-first fasion until all file and directory names have been printed.
When printing the contents of a directory, first, print the word Directory followed by a space and then the full path for the directory you're printing, and ending it with a newline. 
Second, print each of the entries in that directory. Sorted each entry using std::strcmp and print them in that order. 
Each entry will include the inode number, a tab, the name of the entry, and finishing it off with a newline. 
Third, print a empty line consisting of only a newline to finish printing the contents of the directory.
...
bool compareEntryNames(const dir_ent_t &a, const dir_ent_t &b) {  //compare the names of the entries, this is too see like if the name is already in the directory
    return strcmp(a.name, b.name) < 0; 
}

void printDirectoryContents(LocalFileSystem &fs, int inodeNumber, const string &path) { //this function will print the contents of the directory, it will be like recursive function
    ...
    if (ret == -EINVALIDINODE) { 
        cout << "stat failed with " << ret << endl; 
        return; 
    }
    ...
    for (const dir_ent_t &entry : entries) { //print the entries in the directory
    ...
    for (const dir_ent_t &entry : entries) { 
        ...
        if (inode.type == UFS_DIRECTORY && strcmp(entry.name, ".") && strcmp(entry.name, "..")) {
            printDirectoryContents(fs, entry.inum, path + entry.name + "/"); //call the recursive function
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printDirectoryContents(fs, UFS_ROOT_DIRECTORY_INODE_NUMBER, "/"); //recursive function call to print all directories and files
