...
/*
The ds3bits utility prints metadata for the file system on a disk image. 
It takes a single command line argument: the name of a disk image file.
First, it prints metadata about the file system starting with the string Super on a line, then inode_region_addr followed by a space and then the inode region address from the super block. 
Next, it should print the string data_region_addr followed by a space and the data region address from the super block, followed by a newline. 
Finally, print an empty line with just a newline character.
Next it prints the inode and data bitmaps. Each bitmap starts with a string on its own line, Inode bitmap and Data bitmap respectively. For each bitmap, print the byte value, formatted as an unsigned int followed by a space. 
For each byte in your bitmap your print statement might look something like this:
...
int main(int argc, char *argv[]) { 
  ...
  if (argc != 2) { 
    ...
    //Print the metadata about the file system this is the format that the output should be in
    cout << "Super" << endl
